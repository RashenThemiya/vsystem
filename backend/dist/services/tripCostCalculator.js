...
export const calculateActualTripCost = (trip, end_meter, actualReturnDate // ✅ optional parameter
) => {
    ...
    const returnDate = actualReturnDate || trip.actual_return_datetime || new Date();
    ...
    const driverDailyCost = trip.driver_required === "Yes"
        ? Number(trip.driver?.driver_charges || 0)
        : 0;
    const damageCost = Number(trip.damage_cost || 0);
    const otherCosts = (trip.other_trip_costs || []).reduce((sum, cost) => sum + Number(cost.cost_amount || 0), 0);
    ...
    // Gross cost
    const grossTripAmount = vehicleDaily * numDays +
        defaultDistanceCost +
