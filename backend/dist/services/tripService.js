...
export const updateTripService = async (id, data) => {
    ...
    for (const key of fields) {
        if (data[key] !== undefined) {
            updateData[key] = Number.isNaN(Number(data[key]))
                ? data[key]
                : Number(data[key]);
        }
    }
    // Dates
    if (data.leaving_datetime)
        updateData.leaving_datetime = new Date(data.leaving_datetime);
    ...
    // Status
    if (data.trip_status)
        updateData.trip_status = data.trip_status;
...
export const getAllTripsService = async (filters) => {
    const where = {};
    // Filter by status
...
// ========================= START TRIP =========================
export const startTripService = async (trip_id, start_meter) => {
...
// ========================= GET SINGLE TRIP =========================
export const getTripByIdService = async (trip_id) => {
    ...
    if (!trip)
        throw new Error("Trip not found");
    const toBase64 = (bytes) => bytes ? Buffer.from(bytes).toString("base64") : null;
    return {
        ...trip,
...
export const completeTripService = async (trip_id) => {
    const trip = await prisma.trip.findUnique({
        where: { trip_id },
    });
    if (!trip)
 ...
 */
export const addTripPaymentService = async (trip_id, amount, payment_date) => {
    // 1️⃣ Find the trip
    const trip = await prisma.trip.findUnique({
        where: { trip_id },
...
export const cancelTripService = async (trip_id) => {
    // 1️⃣ Fetch the trip
    const trip = await prisma.trip.findUnique({
        where: { trip_id },
