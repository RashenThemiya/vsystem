...
import { calculateActualTripCost } from "./tripCostCalculator.js";
import { PaymentStatus } from "@prisma/client";
export const endTripService = async (trip_id, data) => {
    ...
    const costData = calculateActualTripCost(trip, end_meter);
    // Update trip
...
export const addDamageCostService = async (trip_id, damage_amount) => {
    if (damage_amount <= 0)
...
export const updateTripDatesService = async (trip_id, data) => {
    ...
    const costData = calculateActualTripCost(tripForCalc, updatedTrip.end_meter ?? updatedTrip.start_meter ?? 0);
    const newTotalCost = costData.totalActualCost;
...
export const updateTripMeterService = async (trip_id, data) => {
    const trip = await prisma.trip.findUnique({
        where: { trip_id },
        include: {
            vehicle: true,
    ...
    const paid = Number(updatedTrip.payment_amount || 0);
    let paymentStatus = PaymentStatus.Unpaid;
    if (paid >= costData.totalActualCost)
        paymentStatus = PaymentStatus.Paid;
    else if (paid > 0)
        paymentStatus = PaymentStatus.Partially_Paid;
...
//# sourceMappingURL=endtripService.js.map